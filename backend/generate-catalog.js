...
async function processInBatches(entries, batchSize, worker) {
    for (let i = 0; i < entries.length; i += batchSize) {
        ...
        console.log(`Processed ${Math.min(i + batch.length, entries.length)} / ${entries.length}`);
    }
}
...
async function collectMovieIds() {
    ...
    for (const list of movieLists) {
        ...
        for (let page = 1; page <= 50; page++) {
            ...
            for (const movie of movies) {
                const flags = movieIds.get(movie.id) || { featured: false, trending: false, popular: false };
    ...
    const languages = ["hi", "ko", "ja", "fr", "es", "de", "it", "zh"];
    for (const lang of languages) {
        console.log(`   → discover:${lang}`);
...
async function collectTVIds() {
    ...
    for (const list of tvLists) {
        ...
        for (let page = 1; page <= 50; page++) {
            ...
            for (const show of shows) {
                const flags = tvIds.get(show.id) || { featured: false, trending: false, popular: false };
                flags[list.flag] = true;
    ...
    for (const lang of languages) {
        console.log(`   → discover:${lang}`);
        for (let page = 1; page <= 10; page++) {
            const shows = await fetchDiscoverTV({
                with_original_language: lang,
                sort_by: "popularity.desc",
                "vote_count.gte": 50
            }, page);
...
async function buildMovies() {
    ...
    const { CONCURRENT_REQUESTS } = require("./lib/constants");
    ...
    await processInBatches(
        ...
        async ([id, flags]) => {
            ...
            if (details.vote_count < 100) return;
            if (details.vote_average < 5.0) return;
...
async function buildSeries() {
    ...
    await processInBatches(
        ...
        async ([id, flags]) => {
            ...
            if (!details) return;
            if (!details.poster_path) return;
            if (details.vote_count < 50) return;
            if (details.vote_average < 5.0) return;
...
async function main() {
    ...
    fs.writeFileSync(path.join(outputDir, "movies.json"), JSON.stringify(movies, null, 2));
